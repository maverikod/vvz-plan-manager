...
{b8j5} Concept embeddings are cached in the database in a dedicated cache table whose rows are keyed by the content hash of the embedded serialization; changing a concept invalidates its cached vector automatically. No vector data exists outside the database.
...
{a1z3} `para_list` returns every stored paragraph with its label, binding flag, and position; `para_get` resolves one label to its text. `para_label_assign` finds binding paragraphs that lack labels and inserts fresh unique labels at paragraph start — the only textual edit the server is allowed to make in the HRS. `para_mark_non_binding` is wrap-only: it excludes a stored paragraph from the binding set (its text stays addressable through the version store); the reverse is impossible because non-binding blocks are not stored — restoring one is done by re-importing a corrected HRS document. All `para_*` mutations leave stored paragraph prose byte-identical.
...
{r9f1} `branch_prompt` assembles the executor prompt for one atomic step deterministically from the tree (AS delta, parent TS and GS, HRS slice by source_labels, MRS excerpt for declared concepts) and returns it with its token estimate against the plan's user-set context budget. The token estimate uses the published default of one token per four characters — `(len(text) + 3) // 4` — replaceable via configuration. `branch_dump` materializes all prompts of a plan as a derived snapshot directory, explicitly marked non-authoritative. `branch_weak` returns branches ranked by ascending semantic index.
...
{s5m8} Prompt assembly is concatenation by reference in a fixed order: (1) MRS excerpt — definitions and relations of exactly the concepts the AS declares, ascending by concept id; (2) HRS slice — binding paragraphs whose labels appear in the parent GS source_labels, in document order; (3) parent GS content; (4) parent TS content; (5) the AS delta. The byte-level rendering is fixed: each part appears under its fixed header (`# MRS excerpt`, `# HRS slice`, `# Global step`, `# Tactical step`, `# Atomic step delta`), step content is rendered as canonical JSON of the step's core fields, MRS relations are included when both endpoints are among the declared concepts, and rendered parts are joined by blank lines. The engine resolves declared references only; it never adds, drops, or reorders content by judgement. The same tree state yields a byte-identical prompt.
...
{y1j5} Implementation context: the server is implemented in Python 3.12 or newer. The production package root is `plan_manager/` at the repository root (no `src/` wrapper); modules are dotted paths beneath it (for example `plan_manager/core/...`, `plan_manager/commands/...`). Atomic steps name their target files relative to the repository root under this package. numpy is an approved runtime dependency for scoring mathematics; other third-party runtime dependencies are limited to the platform adapter, psycopg, pydantic, and PyYAML.
