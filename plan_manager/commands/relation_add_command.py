"""Command: add a new relation between two concepts (relation_add)."""
...
from plan_manager.domain.relation import Relation, RelationValidationError
...
class RelationAddCommand(Command):
    """Add a new MRS relation (C-004) between two concepts under an open cascade (C-016).
    ...
    name = "relation_add"
    version = "1.0.0"
    descr = "Add a new relation between two concepts under an open cascade."
    category = "mrs"
    ...
    def get_schema(cls) -> dict:
        """Return the strict input schema for relation_add."""
        return {
            "type": "object",
            "properties": {
                "plan": {
    ...
    def metadata(cls) -> dict:
        return get_relation_add_metadata(cls)
    ...
    async def execute(
        ...
        Returns:
            SuccessResult with from_concept, to_concept, type, and
            revision_uuid, or ErrorResult with code PLAN_NOT_FOUND,
            CASCADE_CONFLICT, or CONCEPT_NOT_FOUND.
        """
        try:
            with db_connection() as conn:
                ...
                if type not in _RELATION_TYPES:
                    return domain_error(
                        "IMPORT_INVALID",
                        f"type '{type}' is not one of {_RELATION_TYPES}",
                        {"field": "type"},
                    )
                existing_concepts = set(list_concept_ids(conn, p.uuid))
                if from_concept not in existing_concepts:
                    return domain_error(
                        "CONCEPT_NOT_FOUND",
                        f"concept not found: {from_concept}",
                        {"field": "from_concept", "concept_id": from_concept},
                    )
                if to_concept not in existing_concepts:
                    return domain_error(
                        "CONCEPT_NOT_FOUND",
                        f"concept not found: {to_concept}",
                        {"field": "to_concept", "concept_id": to_concept},
                    )
                relation = Relation(from_concept=from_concept, to_concept=to_concept, type=type)
                try:
                    relation_uuid = insert_relation(conn, p.uuid, relation)
                except RelationValidationError as exc:
                    return domain_error("IMPORT_INVALID", str(exc), {"field": "relation"})
                ...
                revision_uuid = cascade_write(
                    ...
                    status_updates,
                    "api",
                    f"relation_add: {from_concept}-{type}->{to_concept}",
                )
                found = any(
                ...
                if not found:
                    raise RuntimeError("relation_add: write verification failed")
