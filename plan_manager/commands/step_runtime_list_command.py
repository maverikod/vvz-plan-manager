"""Command: list runtime parameters for a plan or branch scope."""
...
class StepRuntimeListCommand(Command):
    """List runtime parameters for every step in a scope."""

    name: ClassVar[str] = "step_runtime_list"
    version: ClassVar[str] = "1.0.0"
    descr: ClassVar[str] = "List runtime parameters for plan steps in a scope."
    ...
    async def execute(
        self, plan: str, scope: str | None = None, context: object | None = None
        ...
        try:
            with db_connection() as conn:
                ...
                runtime = {
                    artifact_path_of(nodes, step): {
                        "step_id": step.step_id,
                        "runtime": get_runtime_record(conn, p.uuid, step.uuid),
                    }
                    for step in steps
                }
                return SuccessResult(data={"runtime": runtime})
