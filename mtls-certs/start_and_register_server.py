#!/usr/bin/env python3
"""
Start and Register mTLS Server
Starts a simple mTLS server and registers it with MCP Proxy.
...
class SimpleMTLSServer:
    """Simple HTTPS server with mTLS for testing."""
    
    def __init__(self, host: str = "127.0.0.1", port: int = 8001, server_id: str = "simple-mtls-server"):
        self.host = host
        self.port = port
        self.server_id = server_id
        self.app = FastAPI(
            title="Simple mTLS Test Server",
            description="Basic server for testing mTLS registration with MCP Proxy",
            version="1.0.0"
        )
        self._setup_routes()
        self.server = None
    ...
    def _setup_routes(self):
        ...
        async def health():
            ...
            return {"status": "healthy", "message": "Simple mTLS server is running"}
        ...
        async def root():
            """Root endpoint."""
            return {
                "message": "Simple mTLS Test Server",
    ...
    def start_server_thread(self):
        ...
        def run_server():
            logger.info(f"🚀 Starting simple mTLS server on {self.host}:{self.port}")
            
            # SSL configuration
            ssl_keyfile = "certs/mtls/server/primitive-server.key"
            ssl_certfile = "certs/mtls/server/primitive-server.pem"
    ...
    def _create_ssl_context(self) -> ssl.SSLContext:
        ...
        try:
            ...
            if Path(client_cert_file).exists():
                ...
                logger.warning(f"Client certificate not found: {client_cert_file}")
...
async def main():
    """Main function."""
    logger.info("🚀 Starting and Registering mTLS Server")
    logger.info("=" * 60)
    
    # Configuration
    server_host = "127.0.0.1"
    server_port = 8001
    server_id = "simple-mtls-server"
    server_name = "Simple mTLS Test Server"
    server_description = "Basic test server with mTLS for MCP Proxy registration"
    ...
    # Create and start simple mTLS server
    simple_server = SimpleMTLSServer(
        host=server_host,
        port=server_port,
        server_id=server_id
    )
    ...
    server_thread = simple_server.start_server_thread()
    ...
    logger.info(f"🔍 Testing server health at {server_url}/health...")
    
    try:
        ssl_context = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)
        ssl_context.load_verify_locations("certs/mtls/truststore.pem")
        ssl_context.load_cert_chain("certs/mtls/client/primitive-server.pem")
        ssl_context.verify_mode = ssl.CERT_REQUIRED
    ...
    if server_key:
        ...
        logger.info("🎉 Server is running and registered!")
        logger.info(f"📡 Server available at: {server_url}")
        ...
        try:
            ...
            simple_server.stop_server()
            logger.info("🛑 Server stopped by user")
    else:
        logger.error("❌ Registration failed. Stopping server...")
        simple_server.stop_server()
        return
